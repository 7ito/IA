...
public class CatalogUI extends JFrame implements Runnable {
    ...
    public void run() {
        try {
            catalog = new Catalog();
            catalog.getCatalog();
            Object[] items = new ButtonItem[catalog.getItems().size()];
            for (int i = 0; i < items.length; i++) {
                items[i] = new ButtonItem(catalog.getItems().get(i));
            }

            catalogList = new JList(items);
